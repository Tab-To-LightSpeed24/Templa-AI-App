...
const calculateLayoutMetrics = (itemCount: number, layout: string) => {
    ...
    if (layout === 'CENTERED') {
        ...
        else if (itemCount <= 5) { fontSize = 20; lineSpacing = 28; } 
...
const exportPPTX = (project: Project) => {
  ...
  project.sections.forEach(section => {
    ...
    if (layout === 'CENTERED') {
        ...
        const cardFill = styles.cardBg;
        const cardText = styles.cardText;

        slide.addText(col1.map(l => ({ text: l, options: { bullet: { code: '2022', color: styles.accent } } })), {
            shape: pres.ShapeType.roundRect, x: 0.5, y: startY, w: 4.3, h: availH * 0.9, fill: { color: cardFill }, color: cardText, fontSize: metrics.fontSize, align: 'left', valign: 'top', margin: 15, fontFace
        });
        slide.addText(col2.map(l => ({ text: l, options: { bullet: { code: '2022', color: styles.accent } } })), {
            shape: pres.ShapeType.roundRect, x: 5.2, y: startY, w: 4.3, h: availH * 0.9, fill: { color: cardFill }, color: cardText, fontSize: metrics.fontSize, align: 'left', valign: 'top', margin: 15, fontFace
        });
        ...
        items.slice(0, 5).forEach((item, i) => {
            const xPos = 0.5 + (i * step) + (step/2);
            const isTop = i % 2 === 0;
            const yText = isTop ? 1.8 : 3.6;
        ...
        slide.addText(items.map(l => ({ text: l, options: { bullet: { code: '2022', color: styles.accent } } })), { 
            x: 0.5, y: 1.8, w: '90%', h: '70%', fontSize: metrics.fontSize, color: styles.text, lineSpacing: metrics.lineSpacing, fontFace
        });
    } else {
         slide.addText(items.map(l => ({ text: l, options: { bullet: { code: '2022', color: styles.accent } } })), { 
            x: 0.5, y: 1.8, w: '90%', h: '75%', fontSize: metrics.fontSize, color: styles.text, valign: 'top', lineSpacing: metrics.lineSpacing, fontFace
        });
    }
  });
...
const exportDOCX = (project: Project) => {
  ...
  const parseTextWithBold = (text: string) => {
      ...
      return parts.map(part => {
          if (part.startsWith('**') && part.endsWith('**')) {
              return new TextRun({ text: part.slice(2, -2), bold: true, size: 12, font: fontFace });
          }
          return new TextRun({ text: part, size: 12, font: fontFace });
      });
  };
  ...
  const docChildren: any[] = [
    new Paragraph({
      ...
      spacing: { after: 400 },
      run: { font: fontFace, color: styles.accent, size: 32, bold: true } // Theme color for title
    }),
  ];
  ...
  project.sections.forEach(section => {
    ...
    docChildren.push(
      new Paragraph({
        ...
        run: { font: fontFace, color: styles.accent, size: 24, bold: true } // Theme color for headers
      })
    );
    ...
    lines.forEach((line) => {
      ...
      if (trimmed.startsWith('## ')) {
        docChildren.push(new Paragraph({ 
            ...
            run: { font: fontFace, color: styles.text, size: 18, bold: true } 
        }));
         ...
         docChildren.push(new Paragraph({ 
             ...
             run: { font: fontFace, color: styles.text, size: 14, bold: true } 
        }));
...
const getThemeStyles = (template: string) => {
    // accentAlt is for Word table headers (lighter hex without # usually, but docx accepts hex)
    const themes: any = {
