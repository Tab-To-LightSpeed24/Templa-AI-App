...
const exportDOCX = (project: Project) => {
  ...
  const parseTextWithBold = (text: string) => {
      ...
      return parts.map(part => {
          if (part.startsWith('**') && part.endsWith('**')) {
              return new TextRun({ text: part.slice(2, -2), bold: true, size: 24, font: fontFace, color: styles.text });
          }
          return new TextRun({ text: part, size: 24, font: fontFace, color: styles.text });
      });
  };
  ...
  Packer.toBlob(doc).then((blob: Blob) => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `${project.title}.docx`; document.body.appendChild(a); a.click(); document.body.removeChild(a); window.URL.revokeObjectURL(url);
  });
};

// --- PPTX EXPORT ---

const calculateLayoutMetrics = (itemCount: number, layout: string, isVerbose: boolean) => {
    let fontSize = 18;
    let lineSpacing = 28; 
    
    if (layout === 'CENTERED') {
        fontSize = itemCount > 3 ? 24 : 32;
        lineSpacing = 36;
    } else if (layout === 'GRID') {
        fontSize = 14;
        lineSpacing = 20;
    } else {
        if (itemCount <= 3) { fontSize = 24; lineSpacing = 36; } 
        else if (itemCount <= 5) { fontSize = 20; lineSpacing = 30; } 
        else { fontSize = 16; lineSpacing = 22; }
        
        if (isVerbose) {
             fontSize = Math.max(12, fontSize - 2);
             lineSpacing = Math.max(16, lineSpacing - 4);
        }
    }
...
const exportPPTX = (project: Project) => {
  ...
  project.sections.forEach(section => {
    ...
    const avgLength = items.reduce((a, b) => a + b.length, 0) / (items.length || 1);
    const isVerbose = avgLength > 60;

    const metrics = calculateLayoutMetrics(items.length, layout, isVerbose);
